...
def scan_albums(root_folder: str) -> list[LocalAlbumData]:
...
def scan_album_in_folder_if_exists(folder_path: str) -> Optional[LocalAlbumData]:
...
def get_supported_audio_files_in_folder(folder_path: str, max_depth=-1) -> list[LocalTrackData]:
...
# private functions
def _get_supported_audio_files_in_folder(folder_path: str, max_depth, current_depth: int = 1) -> list[LocalTrackData]:
...
def _compile_album_data_from_track_data(parent_directory: str, audio_files: list[LocalTrackData]) -> LocalAlbumData:
...
def _does_audio_files_belong_to_one_album_only(audio_files: list[LocalTrackData]) -> bool:
    if not audio_files:
        return False
...
def _scan_albums(folder_path: str, max_depths: dict[str, int]) -> list[LocalAlbumData]:
